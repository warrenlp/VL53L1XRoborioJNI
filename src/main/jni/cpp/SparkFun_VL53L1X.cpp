...
SFEVL53L1X::SFEVL53L1X(std::shared_ptr<frc::I2C> i2cPort, int shutdownPin, int interruptPin)
{
	_i2cPort = std::shared_ptr<frc::I2C>(i2cPort);
	if (shutdownPin > 0)
	{
		_shutdownPin = std::make_shared<frc::DigitalOutput>(shutdownPin);
	}
	if (interruptPin > 0)
	{
		_interruptPin = std::make_shared<frc::DigitalOutput>(interruptPin);
	}
	_device = std::make_shared<VL53L1X>(i2cPort, shutdownPin, interruptPin);
}
