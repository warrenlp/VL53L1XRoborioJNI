...
{
	public:
	SFEVL53L1X(std::shared_ptr<frc::I2C> i2cPort = std::make_shared<frc::I2C>(frc::I2C::Port::kOnboard, VL53L1X_DEFAULT_DEVICE_ADDRESS), int shutdownPin = -1, int interruptPin = -1); //Constructs our Distance sensor without an interrupt or shutdown pin
	// SFEVL53L1X(frc::I2C &i2cPort, int shutdownPin = -1, int interruptPin = -1); //Constructs our Distance sensor without an interrupt or shutdown pin
	...
	private:
	std::shared_ptr<frc::I2C> _i2cPort;
	std::shared_ptr<frc::DigitalOutput> _shutdownPin;
	std::shared_ptr<frc::DigitalOutput> _interruptPin;
	int _i2cAddress = 0x52;
	std::shared_ptr<VL53L1X> _device;
};